...
namespace cldnn {
primitive_type_id gather_nd8::type_id() {
    static primitive_type_base<gather_nd8> instance;
    return &instance;
}

layout gather_nd8_inst::calc_output_layout(gather_nd8_node const& node) {
...
std::string gather_nd8_inst::to_string(gather_nd8_node const& node) {
    ...
    json_composite gather_nd8_info;
    gather_nd8_info.add("input id", input.id());
    gather_nd8_info.add("input shape", node.input(0).get_output_layout().size.to_string());
    gather_nd8_info.add("indices shape", node.input(1).get_output_layout().size.to_string());
    gather_nd8_info.add("indices rank", desc->indices_rank);
    gather_nd8_info.add("batch dims", desc->batch_dims);
    gather_nd8_info.add("output shape", calc_output_layout(node).size.to_string());

    node_info->add("gather_nd8 info", gather_nd8_info);
    node_info->dump(primitive_description);

    return primitive_description.str();
}

gather_nd8_inst::typed_primitive_inst(network& network, gather_nd8_node const& node) : parent(network, node) {}
