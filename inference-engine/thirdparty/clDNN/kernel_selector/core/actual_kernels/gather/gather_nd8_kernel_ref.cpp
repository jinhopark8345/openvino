...
ParamsKey GatherND8KernelRef::GetSupportedKey() const {
    ParamsKey k;
...
CommonDispatchData GatherND8KernelRef::SetDefault(const gather_nd8_params& params, const optional_params&) const {
...
static size_t GetIndicesLastDim(const gather_nd8_params& params) {
    // get indices dims
    auto indices_dims = params.inputs[1].LogicalDims();
...
static size_t GetSliceSize(const gather_nd8_params& params) {
    // get input dims
...
JitConstants GatherND8KernelRef::GetJitConstants(const gather_nd8_params& params) const {
...
bool GatherND8KernelRef::Validate(const Params& p, const optional_params& o) const {
    if (p.GetType() != KernelType:: GATHER_ND8 || o.GetType() != KernelType::GATHER_ND8) {
        return false;
    }

    const gather_nd8_params& params = static_cast<const gather_nd8_params&>(p);
    auto input_dims = params.inputs[0].LogicalDims();
    auto indices_dims = params.inputs[1].LogicalDims();
...
KernelsData GatherND8KernelRef::GetKernelsData(const Params& params, const optional_params& options) const {
    if (!Validate(params, options)) {
        return {};
    }

    KernelData kd = KernelData::Default<gather_nd8_params>(params);
    gather_nd8_params& newParams = *static_cast<gather_nd8_params*>(kd.params.get());
